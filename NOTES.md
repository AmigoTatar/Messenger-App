...
RuStore отклонил (реестр РКН). Магазины не целимся — APK с сайта (`/potok.apk`). Код шагов 0–5 и кнопка скачивания на вебе **сделаны**. APK: `versionCode` 2 / `1.0.1`.
...
| | |
|---|---|
| **Было** | Кнопки «Скачать» / «Поделиться» — `absolute top-0`, padding `.sheet-safe` их не сдвигал. Сохранение в APK: `fetch(S3)` из WebView → CORS, оба пути падали, тост «Не удалось сохранить фото». `requestPermissions` на Android 13+ бесполезен (WRITE_EXTERNAL_STORAGE maxSdk 29) |
| **Стало** | Тулбар в потоке: `pt-[max(2.75rem, safe-area+0.75rem)]`. Нативка качает через `Filesystem.downloadFile` (свой HTTP, не CORS). Documents без диалога прав; если нельзя — Cache + шаринг «Сохранить фото» |
| **Где** | `MessageList.jsx`, `shareImage.js` |
| **APK** | Нужна новая сборка: JS в `webDir`. Новый плагин не нужен |
